...
export function DestinationAndDateStep({
  ...
  return (
    <>
      {/* Versão Desktop */}
      <div 
        className="hidden md:flex h-16 bg-zinc-900 px-4 rounded-xl items-center shadow-shape gap-3 relative overflow-hidden"
        style={{
          boxShadow: `
      ...
      {/* Versão Mobile */}
      <div 
        className="md:hidden bg-zinc-900 px-4 rounded-xl flex flex-col items-stretch shadow-shape gap-4 relative overflow-hidden py-4"
        style={{
          boxShadow: `
            0px 8px 8px rgba(0, 0, 0, 0.1), 
            0px 4px 4px rgba(0, 0, 0, 0.1), 
      ...
      >
        ...
        <button onClick={openDatePicker} disabled={isGuestsInputOpen} className='flex items-center gap-2 text-left relative z-10'>
          <Calendar className="size-5 text-zinc-400 shrink-0" />
          <span className="text-lg text-zinc-400 truncate">
            {displayedDate || "Quando?"}
          </span>
        ...
        {isGuestsInputOpen ? (
          <Button onClick={closeGuestsInput} variant="secondary" className="relative z-10 w-full">
            Alterar local/data
            <Settings2 className='size-5' />
          </Button>
        ) : (
          <Button onClick={openGuestsInput} variant="primary" className="relative z-10 w-full">
            Continuar
            <ArrowRight className='size-5' />
          </Button>
        )}
